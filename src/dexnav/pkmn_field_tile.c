    ...
    while (top_y < bot_y) {
        while (top_x < bot_x) {
            ...
            for (u8 i = 0; i < NPC_STATE_ID_MAX; i++) {
                if (npc_states[i].to.x == top_x && npc_states[i].to.y == top_y) {
                    go_next = true;
                    break;
                }
            }
...
u8 dexnav_picktile(u8 environment, u8 x_size, u8 y_size, bool small_scan) {
    ...
    return pick_tile_screen(target_behaviour, x_size, y_size, &((*DNavState)->tile_x),
    &((*DNavState)->tile_y) , chance, small_scan);
}

u8 shaking_grass(u8 environment, u8 x_size, u8 y_size, bool small_scan) {
    if (dexnav_picktile(environment, x_size, y_size, small_scan)) {

        oe_state.effect_pos.x = (*DNavState)->tile_x;
        oe_state.effect_pos.y = (*DNavState)->tile_y;
